...
@push('meta-seo')
    <title>Products Edit - Rentify</title>
    <meta content="Halaman Products Edit Rentify" name="description">
...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            <form action="{{ route('be.products.edit_action') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            <input type="hidden" id="edt-product_id" name="product_id"
                                                value="{{ $getProducts->id }}">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" id="add-slug" name="slug"
                                                ...
                                                value="{{ $getProducts->slug }}" readonly>
                                            @error('slug')
                                                <div class="alert alert-danger mt-2 py-2">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="col-lg-12 mb-3">
                                        ...
                                        <textarea id="add-description" name="description" class="form-control @error('description') is-invalid @enderror">{{ $getProducts->description }}</textarea>
                                        @error('description')
                                            <div class="alert alert-danger mt-2 py-2">{{ $message }}</div>
                                        @enderror
                                    </div>
                                            ...
                                            <div>
                                                @foreach ($productImages as $image)
                                                    <img src="{{ asset('assets/be/images/products/' . $image->image_name) }}"
                                                        ...
                                                        class="shadow-sm mt-2 mx-1 rounded-3"
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="selectpicker form-control" id="add-category_id" name="category_id"
                                        ...
                                        @foreach ($getCategories as $item)
                                            <option value="{{ $item->id }}" @selected($getProducts->category_id == $item->id)>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <select class="selectpicker form-control" id="add-brand_id" name="brand_id"
                                        ...
                                        @foreach ($getBrands as $item)
                                            <option value="{{ $item->id }}" @selected($getProducts->brand_id == $item->id)>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
