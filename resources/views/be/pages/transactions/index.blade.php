...
@push('meta-seo')
    <title>Transactions - Rentify</title>
    <meta content="Halaman Transactions Rentify" name="description">
                ...
                <div class="card-body pt-3 pb-md-1">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-end">
                            <a href="{{ route('be.transactions.add') }}" class="btn btn-primary">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="ms-1">Add Transaction</span>
                                </div>
                            </a>
                        </div>
                    </div>
...
@section('content')
                        ...
                        <table id="TransactionsTable" class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-dark text-start">Trx id</th>
                                    <th class="text-dark text-start">Total Amount</th>
                                    <th class="text-dark text-start">Already Paid</th>
                                    <th class="text-dark text-start">Products</th>
                                    <th class="text-dark">
                            ...
                            <tbody>
                                @foreach ($getTransactions as $key => $item)
                                    <tr class="align-middle">
                                        ...
                                        <td class="text-dark text-start">
                                            @if ($item->is_paid == 0)
                                                <span class="badge bg-danger-subtle text-danger">
                                                    <iconify-icon icon="solar:close-circle-linear" width="1.3em"
                                                        height="1.3em"></iconify-icon>
                                                </span>
                                            @else
                                                <span class="badge bg-success-subtle text-success">
                                                    <iconify-icon icon="solar:check-circle-linear" width="1.3em"
                                                        height="1.3em"></iconify-icon>
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-end">
                                            <div class="d-flex align-items-center text-center">
                                                <button type="button" onclick="editTransactions({{ $item->id }})"
    ...
    <div class="modal fade" tabindex="-1" id="deleteTransactions" tabindex="-1" aria-labelledby="deleteTransactions"
        aria-hidden="true">
            ...
            <div class="modal-content modal-filled bg-dark">
                <div class="modal-body p-4">
                    <div class="text-center">
                        ...
                        <form action="{{ route('be.transactions.delete_action') }}" method="post">
                            @csrf
                            <input type="hidden" id="del-transaction_id" name="transaction_id" value="">
...
@push('js')
    ...
    <script>
        function editTransactions(id) {
            window.location.replace("{{ route('be.transactions.edit') }}" + "/" + btoa(id));
        }

        function deleteTransactions(id) {
            $('#del-transaction_id').val(id);
        }
    </script>
@endpush
